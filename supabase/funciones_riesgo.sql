...
--     resto de las funciones acá). Devuelve NULL si la marca no está
--     en la tabla (ej. Alfa Romeo, Porsche) — lib/iqi.ts cae a 'C'.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 1) TOP 10 fallas del grupo Marca+Modelo(+Versión)(+Año)
--
...
--    modelo tiene 12 autos o 500. El conteo crudo mide popularidad
--    del modelo, no qué tan seguido falla — mismo sesgo que ya
--    corregimos en tasa_cangrejo_grupo. 10 de 12 autos (83%) es una
--    falla sistemática; 10 de 500 (2%) es anecdótico.
--
--    Numerador: count(DISTINCT stock_id), no count(*). Si un mismo
--    auto volvió 3 veces por lo mismo, cuenta como 1 auto afectado.
--    El total de OTs se devuelve igual como contexto.
--
--    Denominador: `stock` (inventario completo), no los autos con
--    OTs — si no, la incidencia daría siempre altísima, porque el
--    universo serían solo los autos que ya tuvieron problemas.
--
--    Sin umbral de corte: se devuelve el top 10 ordenado por
--    incidencia y quien consuma decide. Antes había una regla
--    absoluta ("más de 3 OTs") que estaba sesgada por tamaño de
--    flota, y después un umbral de % que agregaba un corte
--    arbitrario sin aportar — el ranking ya ordena por relevancia.
--
--    Por qué NO usar split_part por posición: el aux_sku es
--    MARCA-MODELO-VERSION-AÑO separado por '-', pero hay modelos
--    que traen guión propio (ej "BYD-F3-R-1.5 GLI-2014", modelo
--    real = "F3-R"). split_part(aux_sku,'-',2) agarraba solo "F3"
--    y nunca calzaba → siempre daba 0 OTs. Comparando contra el
--    aux_sku completo con LIKE (modelo va literal, no por índice)
--    el problema desaparece sin importar cuántos guiones tenga.
--
--    Si se entrega versión: igualdad exacta a 4 factores.
--    Si no: MARCA-MODELO-%-AÑO (comodín solo en la versión).
-- ------------------------------------------------------------
...
-- dos vivas. Y con dos candidatas que comparten nombres de parámetro,
-- PostgREST no sabe cuál elegir y falla en runtime.
-- La de (text, text, text) es de cuando p_anio se pasaba como texto.
...
create or replace function top_ots_grupo(
  p_marca   text,
  p_modelo  text,
  p_anio    int  default null,
  p_version text default null
...
language sql stable as $$
  with universo as (
    ...
    where (
      case
        when p_version is not null and trim(p_version) <> '' and p_anio is not null then
          upper(s.aux_sku) = upper(
            trim(p_marca) || '-' || trim(p_modelo) || '-' || trim(p_version) || '-' || p_anio::text
          )
        when p_anio is not null then
          upper(s.aux_sku) like upper(trim(p_marca) || '-' || trim(p_modelo) || '-%-' || p_anio::text)
        else
          upper(s.aux_sku) like upper(trim(p_marca) || '-' || trim(p_modelo) || '-%')
      end
    )
  ),
  conteo as (
    ...
    where coalesce(trim(o.work_item_name), '') <> ''
      and (
        case
          when p_version is not null and trim(p_version) <> '' and p_anio is not null then
            upper(o.aux_sku) = upper(
              trim(p_marca) || '-' || trim(p_modelo) || '-' || trim(p_version) || '-' || p_anio::text
            )
          when p_anio is not null then
            upper(o.aux_sku) like upper(trim(p_marca) || '-' || trim(p_modelo) || '-%-' || p_anio::text)
          else
            upper(o.aux_sku) like upper(trim(p_marca) || '-' || trim(p_modelo) || '-%')
        end
      )
...
-- ------------------------------------------------------------
-- 1b) Totales del grupo, como contexto del top 10.
--
--     top_ots_grupo tiene limit 10, así que no sirve para saber
--     cuántas fallas distintas hay en total ni el tamaño del
--     universo. Eso sale de acá, sin límite.
--
--     ots_por_auto se calcula sobre el universo completo
--     (`stock`), no sobre los autos con OTs — antes daba siempre
--     >= 1 por construcción, porque el denominador excluía a los
--     autos sin ninguna OT. Se devuelve autos_con_ots aparte.
--
--     Ojo: work_item_name es el ítem de reparación puntual
--     (FILTRO DE ACEITE, NEUMATICO, PARABRISAS...), ~103 valores
--     posibles. NO es workshop_name, que son las 6 áreas de taller
--     (Mecánica Liviana, Repuestos Exteriores, etc.).
-- ------------------------------------------------------------
...
create or replace function resumen_ots_grupo(
  p_marca   text,
  p_modelo  text,
  p_anio    int  default null,
  p_version text default null
...
-- Refresca el caché de PostgREST: sin esto, la vista/funciones nuevas
-- (insp_dif_sin_version, insp_dif_grupo, grado_iqi_marca) pueden no
-- aparecer de inmediato para la API — mismo síntoma que ya pasó antes
-- con resumen_ots_grupo ("Could not find the function").
