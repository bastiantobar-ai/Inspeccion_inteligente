      ...
      {resultado && !resultado.error && (
        <section className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 sm:p-8 mt-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="border border-gray-100 bg-gray-50/50 rounded-xl p-4">
              <div className="text-xs text-gray-500 mb-1">Tasa de cangrejo · marca</div>
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-bold text-gray-800">
                  {resultado.cangrejo.tasaMarca.tasaMm != null
                    ? `${resultado.cangrejo.tasaMarca.tasaMm}%`
                    : "—"}
                </span>
              </div>
              <div className="text-xs text-gray-500 mt-2">
                {resultado.cangrejo.tasaMarca.cangrejosMm} cangrejos en{" "}
                {resultado.cangrejo.tasaMarca.autosMm} {marca} (todos los modelos)
              </div>
              <div className="text-[11px] text-gray-400 mt-1">
                base global: {resultado.cangrejo.tasaMarca.tasaBase}%
                {resultado.cangrejo.tasaMarca.vecesBase != null &&
                  ` · ${resultado.cangrejo.tasaMarca.vecesBase}x`}
              </div>
            </div>
          </div>
