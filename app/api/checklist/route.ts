...
export async function POST(req: NextRequest) {
  ...
  const dev = (Array.isArray(devsRpc) ? devsRpc[0] : devsRpc) as
    ...
    | undefined;
  const totalDevoluciones = Number(dev?.total_devoluciones ?? 0);
  ...
  return NextResponse.json({
    ...
    inspeccionDiferenciada: inspDif
      ...
      : null,
    stats: {
      // Histórico de devoluciones por marca+modelo (tabla devoluciones_ot).
    ...
    cangrejo: {
      ...
      // Índice de riesgo 0-100: IQI (30%) + tasa (70%). NO es una
