...
export async function POST(req: NextRequest) {
  ...
  const [
    ...
    // Grado A-E de la marca — lookup en `iqi_marca_modelo` (si hay
    // grado propio para MARCA-MODELO) o `iqi_marca`. Ambas se
    // sincronizan desde la hoja "IQI" del ERP; el código NO calcula
    // nada de marca. Sin error-check propio: si la RPC falla o no hay
    // match, calcularIqi() cae a 'C' en vez de romper el checklist.
    supabase.rpc("grado_iqi_marca", { p_marca: marca, p_modelo: modelo }),
  ...
  // Log de la consulta (best-effort: si el insert falla, la respuesta
  // igual sale — solo se loguea el error). Se hace await para que el
  // insert termine antes de que la función serverless se congele.
  const { error: errLog } = await supabase.from("consultas").insert({
    marca,
    modelo,
    version: version ?? null,
    anio,
    km: km ?? null,
    aux_sku: fullAuxSku ?? `${normaliza(marca)}-${normaliza(modelo)}-${anio}`,
    iqi_grado: iqi.grado,
    iqi_puntaje: iqi.puntaje,
    iqi_bucket_anio: iqi.buckets.anio,
    iqi_bucket_km: iqi.buckets.km,
    iqi_bucket_marca: iqi.buckets.marca,
    iqi_parcial: iqi.parcial,
    iqi_marca_con_grado: iqi.marcaConGrado,
  });
  if (errLog) console.error("consultas insert failed:", errLog.message);
